...
// Garante que o supabase esteja inicializado globalmente
...
if (window.supabaseClient) {
  window.supabaseClient.auth.onAuthStateChange(async (event, session) => {
    const isProtectedPage = window.location.pathname.includes('dashboard') || window.location.pathname.includes('checkout') || window.location.pathname.includes('admin') || window.location.pathname.includes('meus-pedidos');
    ...
    if (session) {
      ...
      try {
        // Se estiver na página de login e já estiver logado, redireciona pro dashboard
        ...
        if (profileError && profileError.code === 'PGRST116') {
          // Perfil não existe, cria automaticamente (útil para login com Google)
          const newProfile = {
            id: user.id,
        ...
        // --- ATUALIZAÇÃO DO DOM NO DASHBOARD ---
        if (isProtectedPage) {
          ...
          if (nameEl) {
            ...
            // Inject ADMIN badge if applicable
          ...
          // Função global para renderizar o carrinho
        ...
        // Emite evento para outros scripts que precisem
        ...
        // Em caso de erro crítico, tenta mostrar a tela para não ficar travado
        const loadingState = document.getElementById('loading-state');
        const dashboardContent = document.getElementById('dashboard-content');
        if (loadingState) loadingState.classList.add('hidden');
        if (dashboardContent) dashboardContent.classList.remove('hidden');
      ...
      // Se não estiver logado e estiver na página protegida, redireciona
      if (isProtectedPage) {
  ...
  window.removeFromCart = async (itemId) => {
    ...
    try {
      ...
      // Recarrega os dados do carrinho e atualiza a UI sem recarregar a página
      ...
      if (session) {
        ...
        if (window.renderCartUI) {
          window.renderCartUI(newCartItems);
        }
      }
  ...
  // Fallback: verifica sessão inicial
  window.supabaseClient.auth.getSession().then(({ data: { session } }) => {
    ...
    } else if (window.location.pathname.includes('dashboard') || window.location.pathname.includes('checkout') || window.location.pathname.includes('admin')) {
      window.location.href = '/login.html';
    }
  });
} else {
