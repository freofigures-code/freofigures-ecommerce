(function () {
    ...
    if (methods && methods.length > 0) {
      ...
      container.innerHTML = methods.map(function(card) {
        return '<div class="p-4 border border-white/5 bg-freo-black flex items-center gap-3">'
          ...
          + escapeHTML(card.brand || 'CARD') + '</div>'
          + '<div>'
          + '<p class="font-mono text-sm">•••• •••• •••• ' + escapeHTML(card.last4 || '****') + '</p>'
          + '<p class="font-mono text-[10px] text-freo-light/50">Exp: '
          + escapeHTML(card.exp_month || '**') + '/' + escapeHTML(card.exp_year || '**') + '</p>'
          + '</div></div>';
      }).join('');
    } else {
      container.innerHTML = '';
  ...
  window.renderCartUI = function(items) {
    ...
    var cartTotalSec   = el('cart-total-section');
    ...
    if (items && items.length > 0) {
      ...
      var itemsHTML = items.map(function(item) {
        ...
        var variantHTML = item.variant
          ? '<p class="font-mono text-[10px] text-freo-orange/80 mt-1 uppercase tracking-wider">'
            + escapeHTML(item.variant) + '</p>'
          : '';
      ...
      if (cartTotalSec && cartTotalPrice) {
        cartTotalSec.classList.remove('hidden');
      ...
      if (cartTotalSec) cartTotalSec.classList.add('hidden');
    }
  };
