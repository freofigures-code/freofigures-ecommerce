    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-md border-b border-freo-orange/20 py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <button onClick={onOpenAuth} className="text-freo-light hover:text-freo-orange transition-colors">
          ...
          <button onClick={onOpenCart} className="text-freo-light hover:text-freo-orange transition-colors relative">
            ...
            {cartItems.length > 0 && (
              <span className="absolute -top-2 -right-2 bg-freo-orange text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                {cartItems.reduce((a: number, i: any) => a + i.quantity, 0)}
              </span>
            )}
          </button>
        </div>
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <button onClick={() => { setIsSearchOpen(true); setMobileMenuOpen(false); }} className="text-freo-light hover:text-freo-orange flex items-center gap-2 mt-2 text-lg">
              <Search className="w-6 h-6" /> Buscar Produto
            </button>
              ...
              <form onSubmit={submitSearch} className="p-4 md:p-5">
                <div className="flex items-center gap-3 bg-freo-black border border-white/10 px-4 py-3 focus-within:border-freo-orange transition-colors">
                  ...
                  <input
                    ...
                    placeholder="Digite o produto: creeper, coringa, joker, santo, chaveiro..."
                ...
                <div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[11px] font-mono text-white/35 uppercase tracking-wider">
                  <span>Busca global: todas as categorias, variações e erros simples de escrita.</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="animate-pulse">
                <div className="aspect-square bg-freo-dark/80 mb-4"/>
                <div className="h-3 bg-freo-dark/80 rounded w-1/3 mb-2"/>
                <div className="h-5 bg-freo-dark/80 rounded w-3/4 mb-4"/>
                <div className="h-10 bg-freo-dark/80 rounded"/>
              </div>
            ))}
          </div>
        ) : products.length === 0 ? (
...
const RELATED_SEARCH_TERMS: Record<string, string[]> = {
  games: ['game', 'gamer', 'geek', 'jogo', 'jogos', 'nerd', 'action figure', 'boneco', 'figura'],
  mario: ['super mario', 'nintendo', 'encanador'],
  coringa: ['joker', 'palhaco', 'palhaço', 'vilao', 'vilão'],
  minecraft: ['mine', 'creeper', 'bloco', 'cubinho', 'chaveiro creeper'],
  chaveiro: ['chaveiros', 'keychain', 'pingente'],
  keycap: ['keycaps', 'tecla', 'teclado', 'switch'],
  religioso: ['religiao', 'religião', 'santo', 'santa', 'fe', 'fé', 'catolico', 'católico', 'catolica', 'católica', 'nossa senhora', 'jesus', 'maria', 'cruz', 'sao bento', 'são bento', 'sao miguel', 'são miguel', 'agua benta', 'água benta', 'pia'],
  personalizado: ['personalizada', 'personalizados', 'custom', 'customizado', 'nome', 'numero', 'número', 'presente', 'encomenda'],
  camisa: ['camiseta', 'uniforme', 'futebol', 'brasil', 'selecao', 'seleção'],
  lifestyle: ['casa', 'decoracao', 'decoração', 'utilitario', 'utilitário', 'utensilio', 'utensílio', 'organizador'],
...
const expandSearchWords = (term: string) => {
  const baseWords = normalizeText(term)
    .split(' ')
  ...
  Object.entries(RELATED_SEARCH_TERMS).forEach(([main, aliases]) => {
    ...
    const shouldExpand = baseWords.some(word =>
      group.some(item => item === word || item.includes(word) || word.includes(item))
    );

    if (shouldExpand) {
      group.forEach(item => {
        item.split(' ').forEach(word => {
          if (word.length > 1 && !SEARCH_STOP_WORDS.has(word)) {
            expanded.add(word);
          }
        });
      });
...
const getSearchScore = (product: Product, searchTerm: string) => {
  ...
  const productWords = text.split(' ').filter(Boolean);
  ...
  if (text.includes(query)) score += 90;
          ...
          <div className="sticky top-32">
            ...
            {hasSearch && (
              <div className="mb-5 bg-freo-orange/10 border border-freo-orange/30 p-4">
                <div className="font-mono text-[10px] text-freo-orange uppercase tracking-widest mb-2">Busca ativa</div>
                <div className="font-display font-bold text-white leading-tight mb-3 break-words">{searchTerm}</div>
                <button onClick={onClearSearch} className="font-mono text-[11px] uppercase text-white/50 hover:text-freo-orange transition-colors">
                  Limpar busca
                </button>
              </div>
            )}
          ...
          <div className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-white/10 pb-4">
            <div className="font-mono text-freo-light/50 text-sm">
              {loading ? 'Carregando...' : hasSearch ? (
                <>Mostrando {filtered.length} resultado{filtered.length !== 1 ? 's' : ''} para a busca <span className="text-freo-orange">[{searchTerm}]</span> em todas as categorias</>
              ) : (
                <>Mostrando {filtered.length} resultado{filtered.length !== 1 ? 's' : ''} para <span className="text-freo-orange">[{categoryLabels[activeFilter] || activeFilter}]</span></>
              )}
            </div>

            {hasSearch && (
              <button onClick={onClearSearch} className="self-start md:self-auto text-xs font-mono uppercase tracking-widest border border-freo-orange/30 text-freo-orange px-3 py-2 hover:bg-freo-orange hover:text-freo-black transition-colors">
            ...
            <div className="text-center py-24">
              ...
              <p className="font-mono text-sm text-white/20">
                {products.length === 0
                  ...
                  : hasSearch
                    ? 'Tente buscar por outra palavra relacionada, exemplo: joker, coringa, creeper, chaveiro, santo, cruz.'
      ...
      try {
        ...
        if (error) {
          ...
          fetch('https://n8nwebhook.solviaoficial.com/webhook/26afb276-b7a5-4d0c-b733-03c47836bd14', {
            ...
            body: JSON.stringify({ name, phone, email, action: 'create_account' })
          }).catch(console.error);
        }
      } catch {
              ...
              {success ? (
                ...
                <div className="space-y-4">
                  ...
                  <form className="space-y-4" onSubmit={handleSubmit}>
                    {!isLogin && (
                      <>
                        <input type="text" placeholder="NOME COMPLETO" value={name} onChange={e => setName(e.target.value)} className="w-full bg-freo-black border border-white/10 px-4 py-3 font-mono text-sm focus:outline-none focus:border-freo-orange transition-colors"/>
                        <input type="tel" placeholder="TELEFONE / WHATSAPP" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-freo-black border border-white/10 px-4 py-3 font-mono text-sm focus:outline-none focus:border-freo-orange transition-colors"/>
                      </>
                    )}
  ...
  const addToCart = async (product: Product) => {
    ...
    try {
      ...
      setCartItems(prev => {
        ...
        if (ex) {
          return prev.map(i => i.cartItemId === data.id ? { ...i, quantity: i.quantity + 1 } : i);
        }
